...
# check if two segments are part of the same scene or non-scene
...
# Function to parse the TEI XML and group scenes/non-scenes based on diegetic levels and properties
def parse_updated_scene_structure(soup, fs_mapping, full_text):
    ...
    # Step 1: Extract the segments and group based on scene/non-scene tags and diegetic level
    ...
    for segment in segments:
        ...
        # Extract the text from the full text using the char range
        segment_text = extract_text_from_char_range(full_text, char_range)

        # Create the current segment
        current_segment = {
            'text': segment_text,
            'diegetic_level': diegetic_level,
            'metadiegetic_level': metadiegetic_level,
...
output_file_path = 'scene_classification.json'
save_segments_to_json(merged_segments_by_scene, output_file_path)

# Check output (optional)
for segment in merged_segments_by_scene[:5]:  # Print first 5 segments for review
    print(segment)
